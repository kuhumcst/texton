...
function scripinit($inputF,$input,$output)  /* Initialises outputfile. */
    {
    global $fscrip, $json2orgfile;
    $fscrip = fopen($json2orgfile,'w');
    if($fscrip)
        {
        fwrite($fscrip,"/*\n");
        fwrite($fscrip," * ToolID           : json2org\n");
        fwrite($fscrip," * Version          : 1\n");
        fwrite($fscrip," * Title            : JSON to ORG-mode\n");
        fwrite($fscrip," * ServiceURL       : http://localhost/json2org\n");
        fwrite($fscrip," * Publisher        : CST\n");
        fwrite($fscrip," * ContentProvider  : CST\n");
        fwrite($fscrip," * Creator          : Bart Jongejan\n");
        fwrite($fscrip," * InfoAbout        : -\n");
        fwrite($fscrip," * Description      : Converts JSON output with tokens, lemmas and Part of Speech tags to a three-column ORG-mode table.\n");
 ...
 */
        ...
        $Ifacet_lem_pos_seg_tok__pos_Menota = false;	/* Style of type of content lemmas (lemmaer) and PoS-tags (PoS-tags) and segments (sætningssegmenter) and tokens (tokens) in input is Menota for the PoS-tags (PoS-tags) component if true */
...
*******************************/
        if( hasArgument("Ifacet_lem_pos_seg_tok") )
            ...
            $input = $input . ($Ifacet_lem_pos_seg_tok__pos_Menota ? " \$Ifacet_lem_pos_seg_tok__pos_Menota" : "") ;
            }
...
//        TODO your code!
        ...
        if($mode == 'dry')
        {
            ...
            scrip("../bin/bracmat 'get\$\"json2org.bra\"' \$F '\$json2orgfile'");
        }
        else
        {
