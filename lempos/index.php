...
function scripinit($inputF,$input,$output)  /* Initialises outputfile. */
    {
    global $fscrip, $lemposfile;
    $fscrip = fopen($lemposfile,'w');
    if($fscrip)
        {
        fwrite($fscrip,"/*\n");
        fwrite($fscrip," * ToolID           : lempos\n");
        fwrite($fscrip," * Version          : 1\n");
        fwrite($fscrip," * Title            : LemPoS\n");
        fwrite($fscrip," * ServiceURL       : http://localhost/lempos\n");
        fwrite($fscrip," * Publisher        : Bart Jongejan\n");
        fwrite($fscrip," * ContentProvider  : Bart Jongejan\n");
        fwrite($fscrip," * Creator          : Bart Jongejan\n");
        fwrite($fscrip," * InfoAbout        : Bart Jongejan\n");
        fwrite($fscrip," * Description      : Lemmatizes input text and adds PoS-options to each lemma. Output can be ambiguous.\n");
...
************************/
        ...
        if( hasArgument("Oambig") )
            ...
            $output = $output . ($Oambigamb ? " \$Oambigamb" : "") ;
            }
        if( hasArgument("Oapp") )
            {
        ...
        if($Oformatjson)
            {
                if($mode == 'dry')
                {
                    if($Iformatflat)
                        ...
                        scrip("../bin/bracmat 'get\$\"LemmaVal.bra\"' '$traindata' '\$IfacetsegF' '\$IfacettokF' '$flexrules' '\$lemposfile' '$TorC'");
                }
                else
                {
