...
Version        : 1.0
Title          : TEI-segmenter
Path in URL    : seg/	*** TODO make sure your web service listens on this path and that this script is readable for the webserver. ***
...
Description    : Reads tokens and sentences as annotations and produces segment annotations, where segments refer to tokens, not to the base text. Input and output is encoded in TEI P5.
ExternalURI    :
XMLparms       :
PostData       :
Inactive       :
...
function scripinit($inputF,$input,$output)  /* Initialises outputfile. */
    {
    global $fscrip, $segfile;
    $fscrip = fopen($segfile,'w');
    if($fscrip)
        {
        fwrite($fscrip,"/*\n");
        fwrite($fscrip," * ToolID           : seg\n");
        fwrite($fscrip," * Version          : 1.0\n");
        fwrite($fscrip," * Title            : TEI-segmenter\n");
        fwrite($fscrip," * ServiceURL       : http://localhost/seg/\n");
        ...
        fwrite($fscrip," * InfoAbout        : http://cst.dk/online/TEIP5-segmenter/\n");
        fwrite($fscrip," * Description      : Reads tokens and sentences as annotations and produces segment annotations, where segments refer to tokens, not to the base text. Input and output is encoded in TEI P5.\n");
 ...
 */
        ...
        $IfacetsentF = "";	/* Input with type of content sentences (sætninger, før tokenisering) */
...
*********/
        if( hasArgument("IfacetsentF") )
            ...
            if($IfacetsentF == '')
                {
                header("HTTP/1.0 404 Input with type of content 'sentences (sætninger, før tokenisering)' not found (IfacetsentF parameter). ");
                return;
                }
...
************************/
        if( hasArgument("Ifacet") )
            ...
            $input = $input . ($Ifacetsent ? " \$Ifacetsent" : "")  . ($Ifacettok ? " \$Ifacettok" : "") ;
            }
        ...
        if( hasArgument("Ofacet") )
            ...
            $output = $output . ($Ofacetseg ? " \$Ofacetseg" : "") ;
            }
...
//        TODO your code!
        ...
        if($mode == 'dry')
            ...
            scrip("../bin/bracmat \"get'\\\"seg.bra\\\"\" \$IfacettokF \$IfacetsentF \$segfile");
            }
        else
            {
            ob_start();
            ...
            $command = "../bin/bracmat \"get'\\\"seg.bra\\\"\" $IfacettokF $IfacetsentF $segfile";
            logit($command);
