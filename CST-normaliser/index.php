...
function scripinit($inputF,$input,$output)  /* Initialises outputfile. */
    {
    global $fscrip, $CSTNormfile;
    $fscrip = fopen($CSTNormfile,'w');
    if($fscrip)
        {
        fwrite($fscrip,"/*\n");
        fwrite($fscrip," * ToolID           : CST-Norm\n");
        fwrite($fscrip," * Version          : 8.2.1\n");
        fwrite($fscrip," * Title            : Normaliser\n");
        fwrite($fscrip," * ServiceURL       : http://localhost/CST-normaliser/\n");
        ...
        fwrite($fscrip," * Description      : Normalises older (1200-1900) Danish text to spelling rules as employed in ODS (Ordbog over det danske Sprog).\n");
...
************************/
        ...
        if( hasArgument("Ifacet") )
            ...
            $input = $input . ($Ifacet_seg_tok ? " \$Ifacet_seg_tok" : "")  . ($Ifacettok ? " \$Ifacettok" : "") ;
            }
        ...
        if( hasArgument("Iperiod") )
            ...
            $input = $input . ($Iperiodc13 ? " \$Iperiodc13" : "")  . ($Iperiodc20 ? " \$Iperiodc20" : "") ;
            }
        ...
        if( hasArgument("Operiod") )
            ...
            $output = $output . ($Operiodc13 ? " \$Operiodc13" : "")  . ($Operiodc20 ? " \$Operiodc20" : "") ;
            }
...
//        TODO your code!
        ...
        if($mode == 'dry')
            {
            if($Opresalf)
                scrip("$cstlemma -L -s' ;; ' -I'\$w\\s' -eU -p -qwft -t$toptarg $Uu -H0 -B'\$f \$w (\$W)\\n' -b'\$f \$w (\$W)\\n' -W'\$f \$w' -l- -f'$toolres$language/lemmatiser/$foptarg$periodsubdir$flexrulessubdir$flexrules' -d'$toolres$language/lemmatiser/$foptarg$periodsubdir$dict' -i \$F -o \$CSTNormfile");
            else if($Opresfrq)
                scrip("$cstlemma -L -s' ;; ' -I'\$w\\s' -eU -p -qfwt -t$toptarg $Uu -H0 -B'\$f \$w (\$W)\\n' -b'\$f \$w (\$W)\\n' -W'\$f \$w' -l- -f'$toolres$language/lemmatiser/$foptarg$periodsubdir$flexrulessubdir$flexrules' -d'$toolres$language/lemmatiser/$foptarg$periodsubdir$dict' -i \$F -o \$CSTNormfile");
            else
                scrip("$cstlemma -L -s' ;; ' -I'\$w\\s' -eU -p -q-   -t$toptarg $Uu -H0 -B'\$w'              -b'\$w'   -c'\$b[[\$b0]?\$B]\$s' -l- -f'$toolres$language/lemmatiser/$foptarg$periodsubdir$flexrulessubdir$flexrules' -d'$toolres$language/lemmatiser/$foptarg$periodsubdir$dict' -i \$F -o \$CSTNormfile");
            }
        else
            {
            if($Opresalf)
                ...
                $command = "$cstlemma -L -s' ;; ' -I'\$w\\s' -eU -p -q-   -t$toptarg $Uu -H0 -B'\$w'              -b'\$w'   -c'\$b[[\$b0]?\$B]\$s' -l- -f'$toolres$language/lemmatiser/$foptarg$periodsubdir$flexrulessubdir$flexrules' -d'$toolres$language/lemmatiser/$foptarg$periodsubdir$dict' -i $F -o $CSTNormfile";
