...
function scripinit($inputF,$input,$output)  /* Initialises outputfile. */
    {
    global $fscrip, $pdf2htmlEXfile;
    $fscrip = fopen($pdf2htmlEXfile,'w');
    if($fscrip)
        {
        fwrite($fscrip,"/*\n");
        fwrite($fscrip," * ToolID           : pdf2htmlEX\n");
        fwrite($fscrip," * Version          : 0.18.8\n");
        fwrite($fscrip," * Title            : pdf2htmlEX\n");
        fwrite($fscrip," * ServiceURL       : http://localhost/pdf2htmlEX/\n");
        fwrite($fscrip," * Publisher        : GitHub\n");
        fwrite($fscrip," * ContentProvider  : GitHub\n");
        fwrite($fscrip," * Creator          : Lu Wang/Trent Petersen\n");
        fwrite($fscrip," * InfoAbout        : https://github.com/pdf2htmlEX\n");
        fwrite($fscrip," * Description      : Converts PDF to HTML without losing text or format. (The produced HTML can hardly be interpreted by other tools.)
Renders PDF files in HTML, utilizing modern Web technologies. It aims to provide an accurate rendering, while keeping optimized for Web display. Best for text-based PDF files, for example scientific papers with complicated formulas and figures. Text, fonts and formats are natively preserved in HTML such that you can still search and copy. The generated HTML file is static, with optional features powered by JavaScript.\n");
...
//        TODO your code!
        ...
        if($mode == 'dry')
            ...
            scrip("\$basepdf2htmlEXfile = basename(\$pdf2htmlEXfile);");
            scrip("LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:/usr/local/lib /usr/local/bin/pdf2htmlEX \$F \$basepdf2htmlEXfile --decompose-ligature 1  --dest-dir /tmp");
            }
        else
            {
