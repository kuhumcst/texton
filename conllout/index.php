...
* helper functions *
*******************/
$toollog = '../log/conllout.log'; /* Used by the logit() function. TODO make sure the folder exists and is writable. Adapt if needed */
...
function scripinit($inputF,$input,$output)  /* Initialises outputfile. */
    {
    global $fscrip, $conlloutfile;
    $fscrip = fopen($conlloutfile,'w');
    if($fscrip)
        {
        fwrite($fscrip,"/*\n");
        fwrite($fscrip," * ToolID           : conllout\n");
        fwrite($fscrip," * Version          : 0.1\n");
        fwrite($fscrip," * Title            : CoNLL formatter\n");
        fwrite($fscrip," * ServiceURL       : http://localhost/conllout/\n");
        fwrite($fscrip," * Publisher        : Københavns Universitet\n");
        fwrite($fscrip," * ContentProvider  : Københavns Universitet\n");
        fwrite($fscrip," * Creator          : Bart Jongejan\n");
        fwrite($fscrip," * InfoAbout        : http://nextens.uvt.nl/depparse-wiki/DataFormat\n");
        fwrite($fscrip," * Description      : Converts input to CoNLL 2009 format.\n");
    ...
    function requestFile($requestParm) // e.g. "IfacettokF"
        ...
        if(isset($_REQUEST[$requestParm]))
            ...
            if($handle == false)
                ...
                {
                $tempfilename = tempFileName("conllout_{$requestParm}_");
...
/* DUMMY CODE TO SANITY CHECK GENERATED SCRIPT (TODO Remove one of the two solidi from the beginning of this line to activate your own code)
        ...
        $command = "echo $echos >> $conlloutfile";
        logit($command);
...
//        TODO your code!
        ...
        if($mode == 'dry')
            ...
            scrip("../bin/bracmat 'get\$\"../shared_scripts/conlln.bra\"' '\$conlloutfile' '\$IfacettokF' '\$IfacetsegF' '\$IfacetposF' '\$IfacetlemF'");
            }
        else
            {
...
// YOUR CODE ENDS HERE. OUTPUT EXPECTED IN $conlloutfile
//*/
        $tmpf = fopen($conlloutfile,'r');

        if($tmpf)
            {
            //logit('output from conllout:');
