...
function scripinit($inputF,$input,$output)  /* Initialises outputfile. */
    {
    global $fscrip, $JSON2tabsepfile;
    $fscrip = fopen($JSON2tabsepfile,'w');
    if($fscrip)
        {
        fwrite($fscrip,"/*\n");
        fwrite($fscrip," * ToolID           : JSON2tabsep\n");
        fwrite($fscrip," * Version          : 1.0\n");
        fwrite($fscrip," * Title            : JSON to TSV\n");
        fwrite($fscrip," * ServiceURL       : http://localhost/JSON2tabsep/\n");
        ...
        fwrite($fscrip," * Description      : Convert word-lemma-pos data from JSON to CQP format.\n");
...
function scrip($str) /* TODO send comments and command line instructions. Don't forget to terminate string with new line character, if needed.*/
    {
    global $fscrip, $JSON2tabsepfile;
    $fscrip = fopen($JSON2tabsepfile,'a');
    if($fscrip)
        {
        fwrite($fscrip,$str . "\n");
        fclose($fscrip);
...
//        TODO your code!
        ...
        if($Oformatflat)
            ...
            if($mode == 'dry')
                scrip("../bin/bracmat 'get\$\"j2t.bra\"' '\$F' '\$JSON2tabsepfile'");
            else
                {
